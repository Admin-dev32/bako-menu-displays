...
export default function TvLayout({
  ...
  return (
    <main className="relative isolate flex min-h-screen w-full items-center justify-center overflow-hidden bg-gradient-to-br from-[#050816] via-[#050816] to-[#020617] text-slate-100">
      {/* Glow global */}
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(249,115,22,0.16),transparent_32%),radial-gradient(circle_at_80%_30%,rgba(34,197,94,0.16),transparent_28%)]" />
      {decorLayer ? <div className="absolute inset-0 -z-10 opacity-80">{decorLayer}</div> : null}

      {/* Canvas fijo 16:9 pensado para 2560x1440 */}
      <div className="relative z-10 box-border w-full max-w-[2560px] aspect-[16/9] px-4 py-6 lg:px-10 lg:py-10">
        <div className="grid h-full grid-cols-1 gap-6 rounded-3xl lg:grid-cols-[0.26fr_0.74fr]">
          ...
          {/* PANEL DERECHO – chalkboard con menú + especial vertical */}
          <section className="relative overflow-hidden rounded-3xl border border-white/5 bg-gradient-to-br from-[#060b19]/92 via-[#050713]/92 to-[#04060f]/96 p-6 shadow-[0_24px_80px_-40px_rgba(0,0,0,0.85)] lg:p-8">
            ...
            <div className="relative flex h-full flex-col gap-6">
              ...
              {/* Contenido principal: menú + especial vertical */}
              <div
                className={`flex flex-1 ${alignClassMap[alignItems]} ${justifyClassMap[justifyContent]}`}
              >
                <div className="grid h-full w-full grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1.7fr)_minmax(0,1.1fr)]">
                  {/* Columna izquierda: grid del menú */}
                  <div className="flex h-full flex-col overflow-hidden">
                    {/* El TvMenuGrid va aquí como está */}
                  ...
                  {/* Columna derecha: especial vertical grande */}
                  <div className="h-full">
                    {specialsSection ? (
                      ...
                      <div className="flex h-full items-center justify-center rounded-3xl border border-dashed border-white/20 bg-black/20 text-xs uppercase tracking-[0.25em] text-slate-300">
